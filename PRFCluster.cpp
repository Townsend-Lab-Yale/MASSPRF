/* Updated by Yide Jin, 09/19/2025 */
/***************************************************
 * Function:
 * Input Parameter:
 * Output:
...
// ==== NEW: file-scope helpers (once per .cpp) ====
// Use these in getPolSysRep/getDivSysRep.

namespace {
    // Detect if a codon contains an alignment gap '-'
    inline bool has_gap(const std::string& c) {
        return c.find('-') != std::string::npos;
    }

    // For -n 2: compute a clear majority base (A/T/G/C) at a codon site
    // across a set of codons. Returns '\0' if there is no clear majority.
    inline char majority_base_at_site(const std::vector<std::string>& codons, int site) {
        int cA=0, cT=0, cG=0, cC=0;
        for (const auto& w : codons) {
            if ((int)w.size() <= site) continue;
            char b = w[site];
            if      (b=='A') ++cA;
            else if (b=='T') ++cT;
            else if (b=='G') ++cG;
            else if (b=='C') ++cC;
            // ignore '-', 'N', and any non-ATGC
        }
        int mx = std::max(std::max(cA, cT), std::max(cG, cC));
        int winners = (cA==mx) + (cT==mx) + (cG==mx) + (cC==mx);
        if (mx==0 || winners!=1) return '\0'; // no clear majority
        if (cA==mx) return 'A';
        if (cT==mx) return 'T';
        if (cG==mx) return 'G';
        return 'C';
...
int PRFCluster::Run(int argc, const char*argv[]) {	
	...
	try {
        ...
        if (const char* gt = std::getenv("GAP_THRESHOLD")) {
...
string PRFCluster::getPolSysRep(vector<string> seq) {
    ...
    for (i=0; i<seq_length; i+=CODONSIZE) {
        ...
        for (j=0; j<(long)seq.size(); j++) {
            string word = seq[j].substr(i, CODONSIZE);
            if ((word[0]=='A'||word[0]=='T'||word[0]=='G'||word[0]=='C') &&
                (word[1]=='A'||word[1]=='T'||word[1]=='G'||word[1]=='C') &&
                (word[2]=='A'||word[2]=='T'||word[2]=='G'||word[2]=='C')) {
                codon.push_back(word);
            } else {
                codon_other.push_back(word);
            }
        }

        // If some of the codons contain letters other than A/T/G/C, they have to be replaced (IUPAC)
        if (codon_other.size()>0) {
            flag_N_pol = 1;
            if (Nuc_replace == 1) {
                // Find the most frequently used nucleotide (A/T/G/C) for each site
                // And replace codons that are not A/T/G/C (IUPAC ambiguity)
                ReplaceCodon4PolSys(codon, codon_other);

                // Merge codon array and codon_other array
                for (j=0; j<(long)codon_other.size(); j++) {
                    codon.push_back(codon_other[j]);
                }
            } else if (Nuc_replace == 0) {
                // Ambiguous nucleotide is seen as gap
                cons_seq += "***";
                continue;
            }
            // else: Nuc_replace==2 will handle gaps in the following "NEW -n 2"
        }

        // =============================== //
        // **NEW (1/2)**  -n 2: majority-impute gaps (only if a clear per-site majority exists)
        //   - Works after IUPAC handling so -n keeps its intended semantics.
        //   - If any site has no clear majority, we mask this codon as *** to stay conservative.
        // =============================== //
        if (Nuc_replace == 2) {
            // Pool codons (post-IUPAC) for majority vote
            std::vector<std::string> all_codons;
            all_codons.reserve(codon.size() + codon_other.size());
            for (const auto& c : codon)       all_codons.push_back(c);
            for (const auto& c : codon_other) all_codons.push_back(c);

            // Compute per-site majority among A/T/G/C
            char m0 = majority_base_at_site(all_codons, 0);
            char m1 = majority_base_at_site(all_codons, 1);
            char m2 = majority_base_at_site(all_codons, 2);

            bool unresolved = false;
            for (auto& c : all_codons) {
                for (int s=0; s<3; ++s) {
                    if (c[s]=='-') {
                        char m = (s==0?m0:(s==1?m1:m2));
                        if (m!='\0') c[s]=m;        // safe to impute
                        else { unresolved = true; break; } // no clear majority → stop
                    }
                }
                if (unresolved) break;
            }
            ...
            if (unresolved) {
                cons_seq += "***";   // too ambiguous → mask the whole codon
                continue;
            }

            // Accept imputed, gap-free codons for downstream S/R logic
            codon.swap(all_codons);
            codon_other.clear();
        }
        // =============================== //
        // **END NEW (1/2)**
        ...
        // =============================== //
        // **NEW (2/2)**  gap handling (pairwise/threshold)
        //  - Uses only gap-free A/T/G/C codons for S/R decisions
        //  - Honors gap_policy:
        //      0: skip any '-' (mask)
        //      1: pairwise (need >=2 valid codons)
        //      2: pairwise + threshold (mask if gap_frac > gap_threshold)
        // =============================== //
        {
            const int total_n = (int)seq.size();
            std::vector<std::string> valid_codons;
            valid_codons.reserve(total_n);

            int gap_count = 0;
            // Re-scan all sequences at this codon to build gap-free set
            for (int jj = 0; jj < total_n; ++jj) {
                std::string w = seq[jj].substr(i, CODONSIZE);
                if (has_gap(w)) { gap_count++; continue; }
                if ((w[0]=='A'||w[0]=='T'||w[0]=='G'||w[0]=='C') &&
                    (w[1]=='A'||w[1]=='T'||w[1]=='G'||w[1]=='C') &&
                    (w[2]=='A'||w[2]=='T'||w[2]=='G'||w[2]=='C')) {
                    valid_codons.push_back(w);
                }
            }

            const double gap_frac = total_n ? (double)gap_count / (double)total_n : 0.0;

            bool mask_this_codon = false;
            if (gap_policy == 0) {
                // strict skip: any '-' masks
                mask_this_codon = (gap_count > 0);
            } else if (gap_policy == 1) {
                // pairwise: require at least 2 gap-free codons
                mask_this_codon = ((int)valid_codons.size() < 2);
            } else { // gap_policy == 2 : pairwise + threshold
                if (gap_frac > gap_threshold) mask_this_codon = true;
                else                          mask_this_codon = ((int)valid_codons.size() < 2);
            }
            ...
            // Use only gap-free codons for downstream S/R logic
            codon.swap(valid_codons);
        }
        // =============================== //
        // **END NEW (2/2)**
        // =============================== //
...
string PRFCluster::getDivSysRep(vector<string> pol, vector<string> div) {
    ...
    for (i=0; i<seq_length; i+=CODONSIZE) {
        ...
        // Consider about the nucleotide other than A, T, G or C (IUPAC ambiguity)
        if (codon_other.size()>0 ||
            (div_codon[0]!='A'&&div_codon[0]!='T'&&div_codon[0]!='G'&&div_codon[0]!='C') ||
            (div_codon[1]!='A'&&div_codon[1]!='T'&&div_codon[1]!='G'&&div_codon[1]!='C') ||
            (div_codon[2]!='A'&&div_codon[2]!='T'&&div_codon[2]!='G'&&div_codon[2]!='C')) {

            // Replace the ambiguous nucleotide
            if (Nuc_replace==1) {
                // Replace IUPAC in POL/DIV by majority base (A/T/G/C)
            ...
            // else: Nuc_replace==2 will handle gaps just below
        }

        // ===============================
        // **NEW (1/2)**  -n 2: impute gaps in POL + DIV by clear majority
        //   If any site lacks a clear A/T/G/C majority, mask this codon (***)
        // ===============================
        if (Nuc_replace == 2) {
            std::vector<std::string> all_codons;
            all_codons.reserve(codon.size()+1);
            for (const auto& c : codon) all_codons.push_back(c);
            all_codons.push_back(div_codon);
            ...
            // Split back: last is DIV, others are POL
            div_codon = all_codons.back();
            all_codons.pop_back();
            codon.swap(all_codons);
            codon_other.clear();
        }
        ...
        // ================
        // **NEW (2/2)** gap handling (pairwise/threshold) for POL + DIV
        //  - Count gaps in both POL and DIV
        //  - policy 0: any gap -> mask
        //  - policy 1: pairwise (need ≥2 gap-free POL) AND DIV must be gap-free
        //  - policy 2: threshold on gap_frac + pairwise condition
        // ================
        {
            const int total_n = (int)pol.size() + 1; // +1 for divergence
            std::vector<std::string> valid_codons;   // gap-free POL codons
            valid_codons.reserve(pol.size());

            int gap_count = 0;
            if (has_gap(div_codon)) gap_count++; // count DIV gaps

            for (int jj = 0; jj < (int)pol.size(); ++jj) {
                std::string w = pol[jj].substr(i, CODONSIZE);
                if (has_gap(w)) { gap_count++; continue; }
                if ((w[0]=='A'||w[0]=='T'||w[0]=='G'||w[0]=='C') &&
                    (w[1]=='A'||w[1]=='T'||w[1]=='G'||w[1]=='C') &&
                    (w[2]=='A'||w[2]=='T'||w[2]=='G'||w[2]=='C')) {
            ...
            if (gap_policy == 0) {
                ...
                mask_this_codon = ((int)valid_codons.size() < 2) || has_gap(div_codon);
            } else { // gap_policy == 2
                if (gap_frac > gap_threshold) mask_this_codon = true;
                else                          mask_this_codon = ((int)valid_codons.size() < 2) || has_gap(div_codon);
            }
            ...
            // Use only gap-free POL codons; DIV is guaranteed gap-free here
            codon.swap(valid_codons);
        }
        // ===== **END NEW (2/2)** =====
        ...
        for (j=0; j<(long)codon.size() && isDivCodon==true; j++) {
    ...
    throw 1;
		 ...
		 */
			...
			for (i=1; i<argc; i++) {
                ...
                // Replace or treat as gap for ambiguous sites / gaps
                else if (temp=="-N" && (i+1)<argc && nuc_replace_flag==0){
                    int num = CONVERT<int>(argv[++i]);
                    // -N modes:
                    //   0: skip gaps and ambiguous IUPAC sites (mask as ***)
                    //   1: skip gaps; replace IUPAC by majority base
                    //   2: replace gaps and IUPAC by majority base
 ...
 * Output:
 ***************************************************/
// Information and help when type ./MASS-PRF -h 
// Updated 2025/09/19 by Yide Jin
void PRFCluster::showHelpInfo() {
    ...
    cout << "      \tRecommendation: Use `-s 1` for improved accuracy." << endl;
    ...
    cout << "  -mn\tNumber of models in stochastic algorithm [integer, optional when -exact=0], default=10000" << endl;
    ...
    cout << "  -a\tMinimum gene length to trigger scaling [integer, optional]" << endl;
    cout << "  -rMAp\tOutput gamma from model-averaged pr and dr [0 or 1], default=0" << endl;
    cout << "  -SCL\tCompress sequences by units of 3, 6, or 9 nucleotides [integer, optional]" << endl;
    cout << "  -h\tShow help information" << endl;
    cout << endl;

    cout << "NOTES:" << endl;
    cout << "  * Program auto-detects available CPU cores to optimize computation." << endl;
    cout << "  * Runtime will print [GAP] policy and threshold settings for transparency." << endl;
    cout << endl;
    ...
    cout << "CONTACT:" << endl;
    cout << "Please send suggestions or report bugs to:" << endl;
    cout << "  Yide Jin (jinyide0202@gmail.com)" << endl;
    cout << "  Zi-Ming Zhao (ziming.gt@gmail.com)" << endl;
    cout << "  Michael C. Campbell (mc44680@usc.edu)" << endl;
    cout << "  Jeffrey Townsend (Jeffrey.Townsend@Yale.edu)" << endl;
    cout << endl;
}
